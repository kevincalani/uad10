...
export default function TramiteActionButton({type, onClick}) 
{
    ...
    const { base, hover,border } = colorConfig;

    return (
    <button 
      // Usar las constantes importadas
      className={`flex items-center text-black font-semibold py-2 px-4 rounded shadow-md transition duration-150 border ${border} hover:${base} hover:text-white ${hover} `}
